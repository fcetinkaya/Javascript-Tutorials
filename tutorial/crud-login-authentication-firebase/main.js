 ...
 firebase.auth().onAuthStateChanged(function (user) {
    ...
    <p style = "display: none;" id = "taskkey"></p>
        <button type = "submit" id = "button1" class = "btn-block btn btn-primary">Add Task</button>
        <button type = "button" id = "button2" class = "btnop2 btn btn-success" style = "display:none;" onclick = "updateTask(2)">Update Task</button>
        <button type = "button" id = "button3" class = "btnop2 btn btn-danger" style = "display:none;" onclick = "cancel()">Cancel</button>
    </form>
        <h2>Fatih Çetinkaya</h2>
        <h3>Software Developer</h3>
        <button type = "submit" class = "btn-block btn mt-5 btn-danger" onclick = "firebase.auth().signOut()">Sign Out</button>
     </div>
        ...
        <div class = "row" style = "position: fixed; bottom: 5%;">
...
<p id="errorm" class="mt-5 mb-3 text-danger"></p>
<p class = "text-muted" > user: crud@example.com // Psw: 123456 </p>
<p class = "mt-5 mb-3 text-muted">&copy;Fatih Çetinkaya </p>
 ...
 function readTask() {
     firebase.firestore().collection("tasks").onSnapshot(function (snapshot) {
         ...
         snapshot.forEach(function (taskValue) {
             document.getElementById("cardSection").innerHTML += `
            <div class = "card mb-3" style="background-color: inherit; border: 1px solid;">
                <div class = "card-body">
                    <h5 class = "text-white card-title">
                Task: ${taskValue.data().task}</h5>
                 <p class="card-text text-white">Description: ${taskValue.data().description}
